...
const Overview = () => {
  ...
  // ✅ Get videos for this district (filtered from allVideos)
  ...
  const completedVideos = districtVideos.filter(
    (v) => v.status === "completed",
  ).length;
  const processingVideos = districtVideos.filter(
    (v) => v.status === "processing",
  ).length;
  const uploadedVideos = districtVideos.filter(
    (v) => v.status === "uploaded" || v.status === "uploading",
  ).length;
  const failedVideos = districtVideos.filter(
    (v) => v.status === "failed",
  ).length;
  ...
  const fetchData = async () => {
    ...
    try {
      await Promise.all([
        ...
        dispatch(getVideosByDistrict(districtId)), // ✅ This populates allVideos with district videos
      ]);
    } catch (error) {
        ...
        <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-red-600">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">{videosToday} today</p>
            </div>
