...
const Analytics = () => {
  ...
  const derivedVideoStats = useSelector(selectDerivedVideoStats);
  const currentDistrict = useSelector(selectCurrentDistrict);
  
  // Filter shops by current district
  ...
  // Get video stats for a specific shop
  const getShopVideoStats = (shopId) => {
    // Filter videos for this shop
    const shopVideos = allVideos.filter(video => video.shop_id === shopId);
    ...
    return {
      ...
      shopLocation: shop ? `${shop.city}${shop.state ? `, ${shop.state}` : ''}` : '',
      totalVideoRequests: totalVideos,
      uploadedVideos,
      processingVideos,
      completedVideos,
      failedVideos,
      completionRate,
      shopActive: shop?.is_active || false
    };
  };
  ...
  const totalCompletedVideos = allVideos.filter(video => {
    const shop = shops.find(s => s.id === video.shop_id);
    return shop && shop.district_id === districtId && video.status === 'completed';
  }).length;

  const totalFailedVideos = allVideos.filter(video => {
    const shop = shops.find(s => s.id === video.shop_id);
    return shop && shop.district_id === districtId && video.status === 'failed';
  }).length;

  const totalProcessingVideos = allVideos.filter(video => {
    const shop = shops.find(s => s.id === video.shop_id);
    return shop && shop.district_id === districtId && video.status === 'processing';
  }).length;

  const totalUploadedVideos = allVideos.filter(video => {
    const shop = shops.find(s => s.id === video.shop_id);
    return shop && shop.district_id === districtId && video.status === 'uploaded';
  }).length;
    ...
    <div className="p-6">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          {currentDistrict ? `District: ${currentDistrict.name}` : 'Track video processing and performance across shops in your district'}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-blue-50 rounded-lg p-6 border border-blue-100">
          ...
          <p className="text-2xl font-bold text-blue-700">{totalVideosInDistrict}</p>
        ...
        <div className="bg-green-50 rounded-lg p-6 border border-green-100">
          ...
          <p className="text-2xl font-bold text-green-700">{overallCompletionRate}%</p>
        ...
        <div className="bg-yellow-50 rounded-lg p-6 border border-yellow-100">
          ...
          <p className="text-2xl font-bold text-yellow-700">{overallProcessingRate}%</p>
        ...
        <div className="bg-red-50 rounded-lg p-6 border border-red-100">
          ...
          <p className="text-2xl font-bold text-red-700">{totalFailedVideos}</p>
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="text-center p-4 bg-blue-100 rounded-lg">
            <div className="text-2xl font-bold text-blue-600">{totalUploadedVideos || 0}</div>
            <div className="text-sm text-blue-500">Uploaded</div>
          </div>
          
          <div className="text-center p-4 bg-yellow-100 rounded-lg">
            <div className="text-2xl font-bold text-yellow-600">{totalProcessingVideos || 0}</div>
            <div className="text-sm text-yellow-500">Processing</div>
          </div>
          
          <div className="text-center p-4 bg-green-100 rounded-lg">
            <div className="text-2xl font-bold text-green-600">{totalCompletedVideos || 0}</div>
            <div className="text-sm text-green-500">Completed</div>
          </div>
          
          <div className="text-center p-4 bg-red-100 rounded-lg">
            <div className="text-2xl font-bold text-red-600">{totalFailedVideos || 0}</div>
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden mb-8">
        <div className="p-6 border-b">
          ...
          <p className="text-gray-600">Video requests and processing status by shop ({shops.length} shops)</p>
        </div>
      ...
      <div className="mt-8 text-sm text-gray-500 bg-gray-50 p-4 rounded-lg">
        ...
        <p>Total video requests in district: {totalVideosInDistrict}</p>
        <p>Total shops in district: {shops.length}</p>
        <p>Overall completion rate: {overallCompletionRate}%</p>
        <p>Active shops: {shops.filter(s => s.is_active).length}</p>
      </div>
                        ...
                        <div>
                          ...
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div 
                              ...
                              style={{ width: `${shopAnalyticsData.processingVideos / shopAnalyticsData.totalVideoRequests * 100}%` }}
                            ></div>
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div 
                              ...
                              style={{ width: `${shopAnalyticsData.failedVideos / shopAnalyticsData.totalVideoRequests * 100}%` }}
                            ></div>
                          </div>
                        </div>
                      </div>
