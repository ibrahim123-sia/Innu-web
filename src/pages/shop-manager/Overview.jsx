...
const Overview = () => {
  ...
  const calculateStats = () => {
    ...
    if (filteredShopUsers.length > 0 && myShop) {
      const technicians = filteredShopUsers.filter(user => user.role === 'technician').length;
      ...
      const inProgressOrders = shopOrders.filter(order => 
        ['in_progress', 'work-in-progress', 'processing'].includes(order.status?.toLowerCase())
      ).length;
  ...
  {myShop ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <div className="mb-3">
            ...
            <p className="text-gray-600">{myShop.street_address || 'Not provided'}</p>
          </div>
          <div className="mb-3">
            <label className="block text-sm font-medium text-gray-700">City</label>
            <p className="text-gray-600">{myShop.city || 'Not provided'}</p>
          </div>
          <div className="mb-3">
            <label className="block text-sm font-medium text-gray-700">State</label>
            <p className="text-gray-600">{myShop.state || 'Not provided'}</p>
          </div>
        </div>
        <div>
          <div className="mb-3">
          ...
          <div className="mb-3">
            ...
            <p className="text-gray-600">
              {myShop.created_at ? new Date(myShop.created_at).toLocaleDateString('en-US', {
                month: 'short',
                day: 'numeric',
                year: 'numeric'
              }) : 'N/A'}
    ...
    <div className="text-center py-8 border-2 border-dashed border-gray-200 rounded-lg">
      ...
      <p className="text-sm text-gray-400 mt-1">Please contact administrator</p>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow duration-200">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold text-gray-800">Shop Performance</h2>
            {topShop && (
          ...
          {topShop ? (
            <div className="border rounded-lg p-4">
              <div className="flex items-center space-x-4 mb-4">
                ...
                <div>
                  <h3 className="font-bold text-xl text-gray-900">{topShop.name}</h3>
                  <p className="text-sm text-gray-600">
                    {topShop.city}{topShop.state ? `, ${topShop.state}` : ''}
                  </p>
                  <p className="text-xs text-blue-600 mt-1 font-mono">
                    Tekmetric ID: {topShop.tekmetric_shop_id || 'N/A'}
                  </p>
                </div>
              </div>
                ...
                <div className="bg-blue-50 p-3 rounded-lg">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-blue-700">{topShop.totalOrders}</p>
                    </div>
                ...
                <div className="bg-green-50 p-3 rounded-lg">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-green-700">{topShop.completedOrders}</p>
                    </div>
              ...
              <div className="grid grid-cols-3 gap-2 mb-4">
                <div className="bg-yellow-50 p-2 rounded-lg text-center">
                  ...
                  <p className="text-lg font-bold text-yellow-700">{topShop.pendingOrders}</p>
                </div>
                <div className="bg-orange-50 p-2 rounded-lg text-center">
                  <p className="text-xs text-orange-600 font-medium">In Progress</p>
                  <p className="text-lg font-bold text-orange-700">{topShop.inProgressOrders}</p>
                </div>
                <div className="bg-purple-50 p-2 rounded-lg text-center">
                  <p className="text-xs text-purple-600 font-medium">Technicians</p>
                  <p className="text-lg font-bold text-purple-700">{topShop.totalTechnicians}</p>
                </div>
              </div>

              <div className="bg-gray-50 p-3 rounded-lg">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">AI Video Requests</span>
                  <span className="text-lg font-bold text-green-600">{topShop.videoRequests}</span>
                </div>
                <div className="flex justify-between items-center mt-1">
                  <span className="text-sm text-gray-600">Completion Rate</span>
                  <span className="text-lg font-bold text-purple-600">{shopStats?.completionRate || 0}%</span>
                </div>
              </div>
            </div>
          ) : (
            <div className="text-center py-8 border-2 border-dashed border-gray-200 rounded-lg">
              ...
              <p className="text-sm text-gray-400 mt-1">Orders and videos will appear here</p>
            </div>
          )}
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500">View and manage all shop orders</p>
                </div>
              ...
              >
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-gray-800">Manage Technicians</p>
                  <p className="text-xs text-gray-500">Create and manage technicians</p>
                </div>
                <svg className="w-5 h-5 text-gray-400 group-hover:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                </svg>
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500">Check shop performance metrics</p>
                </div>
                <svg className="w-5 h-5 text-gray-400 group-hover:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                </svg>
      ...
      <div className="mt-8 bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow duration-200">
        <div className="flex justify-between items-center mb-4">
          ...
          <Link to="/shop-manager/orders" className="text-blue-600 hover:text-blue-800 hover:underline text-sm font-medium">
            View All
          </Link>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">RO #</th>
