...
const BrandDetailModal = ({ brandId, onClose }) => {
  ...
  const getBrandLogo = () => {
    if (brand?.logo_url?.trim()) return brand.logo_url;
    return DEFAULT_BRAND_LOGO;
  };

  const getAdminProfilePic = () => {
    if (brandAdmin?.profile_pic_url?.trim()) return brandAdmin.profile_pic_url;
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-lg p-8">
          ...
          <p className="mt-4 text-gray-600">Loading companies details...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 text-center">
          ...
          <p className="text-gray-500 mb-4">The company you're looking for doesn't exist or has been removed.</p>
        ...
        <div className="p-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            ...
            <div className="bg-yellow-50 p-4 rounded-lg">
              <h3 className="text-sm text-yellow-600 font-medium">Repair Orders</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            <div className="space-y-4">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div className="text-sm text-gray-600">
                  ...
                  <p className={brand.logo_url?.trim() && brand.logo_url !== DEFAULT_BRAND_LOGO ? 'text-green-600' : 'text-yellow-600'}>
                    {brand.logo_url?.trim() && brand.logo_url !== DEFAULT_BRAND_LOGO ? 'Custom logo uploaded' : 'Using default logo'}
                  </p>
                  {brand.logo_url?.trim() && brand.logo_url !== DEFAULT_BRAND_LOGO && (
                    <a href={brand.logo_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline mt-2 inline-block text-sm">
                      View Logo
                    </a>
                  )}
                </div>
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-bold text-gray-800">Company Admin</h3>
              {brandAdmin.id ? (
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      {brandAdmin.profile_pic_url?.trim() && brandAdmin.profile_pic_url !== DEFAULT_PROFILE_PIC 
                ...
                <div className="flex items-center space-x-4">
                  <div className="w-32 h-32 rounded-full overflow-hidden border bg-gray-100 flex items-center justify-center">
                    <img src={DEFAULT_PROFILE_PIC} alt="No admin" className="w-20 h-20 opacity-50" />
                  </div>
                  <div>
                    <p className="text-gray-500 italic">No admin assigned to this company</p>
                    <p className="text-sm text-gray-400 mt-1">Assign an admin in the Companies page</p>
                  </div>
                </div>
              )}
            </div>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {brandShops.slice(0, 5).map((shop) => (
                      <tr key={shop.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <div className="text-sm text-gray-500 font-mono">{shop.tekmetric_shop_id || 'N/A'}</div>
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap">
          ...
          <div>
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-gray-800">Recent Repair Orders</h3>
            ...
            {brandOrders.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">RO ID</th>
                ...
                {brandOrders.length > 5 && (
                  <p className="text-sm text-gray-500 mt-2 px-4">
                    Showing 5 of {brandOrders.length} repair orders
                  </p>
                )}
              </div>
            ) : (
              <div className="text-center py-8 border-2 border-dashed border-gray-200 rounded-lg">
                ...
                <p className="text-gray-500 italic">No repair orders found for this company</p>
                <p className="text-sm text-gray-400 mt-1">Repair orders will appear here when shops start creating them</p>
              </div>
            )}
          </div>
        </div>
