    ...
    builder
      ...
      .addCase(createDistrict.fulfilled, (state, action) => {
        ...
        if (action.payload?.data) {
          ...
          state.districts.unshift(district);
      ...
      .addCase(updateDistrict.fulfilled, (state, action) => {
        ...
        if (updatedDistrict?.id) {
          const index = state.districts.findIndex(district => district.id === updatedDistrict.id);
          if (index !== -1) state.districts[index] = updatedDistrict;
          
          const brandIndex = state.districtsByBrand.findIndex(district => district.id === updatedDistrict.id);
          if (brandIndex !== -1) state.districtsByBrand[brandIndex] = updatedDistrict;
          
          if (updatedDistrict.brand_id && state._districtsByBrandMap[updatedDistrict.brand_id]) {
            const mapIndex = state._districtsByBrandMap[updatedDistrict.brand_id].findIndex(
              district => district.id === updatedDistrict.id
            );
            if (mapIndex !== -1) state._districtsByBrandMap[updatedDistrict.brand_id][mapIndex] = updatedDistrict;
          }
          
          if (state.currentDistrict?.id === updatedDistrict.id) state.currentDistrict = updatedDistrict;
        }
      ...
      .addCase(deleteDistrict.fulfilled, (state, action) => {
        ...
        if (deletedId) {
          ...
          if (state.currentDistrict?.id === deletedId) state.currentDistrict = null;
        }
      ...
      .addCase(getDistrictsByBrand.fulfilled, (state, action) => {
        ...
        let districts = [];
        if (data?.data && Array.isArray(data.data)) districts = data.data;
        else if (Array.isArray(data)) districts = data;
        else if (data && typeof data === 'object') {
          const possibleArray = Object.values(data).find(val => Array.isArray(val));
          if (possibleArray) districts = possibleArray;
        }
        ...
        districts.forEach(district => {
          ...
          if (!exists) state.districts.push(district);
        });
      })
