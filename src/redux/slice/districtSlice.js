  ...
  async (districtId, { rejectWithValue }) => {
    try {
      const response = await API.delete(
        `/districts/deletedistricts/${districtId}`,
      );
      return response.data;
    } catch (error) {
      return rejectWithValue(error.response?.data || { error: error.message });
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(createDistrict.fulfilled, (state, action) => {
        ...
        state.message =
          action.payload?.message || "District created successfully";
      })
      ...
      .addCase(deleteDistrict.fulfilled, (state, action) => {
        ...
        if (deletedId) {
          ...
          state.districtsByBrand = state.districtsByBrand.filter(
            (district) => district.id !== deletedId,
          );
        ...
        state.message =
          action.payload?.message || "District deleted successfully";
      })
      ...
      .addCase(getDistrictsByBrand.rejected, (state, action) => {
        state.loading = false;
        state.error =
          action.payload?.error || "Failed to fetch districts by brand";
        state.districtsByBrand = [];
      })
      ...
      .addCase(getActiveDistricts.rejected, (state, action) => {
        state.loading = false;
        state.error =
          action.payload?.error || "Failed to fetch active districts";
...
export const selectActiveDistricts = (state) =>
  state.district.activeDistricts || [];
export const selectShopsByDistrict = (state) =>
  state.district.shopsByDistrict || [];
