    ...
    try {
      ...
      if (response.data.data?.token) {
        ...
        localStorage.setItem(
          "is_first_login",
          response.data.data.is_first_login || false,
        );
      }
...
export const toggleUserActiveStatus = createAsyncThunk(
  ...
  async ({ userId, is_active }, { rejectWithValue }) => {
    try {
      const response = await API.patch(`/users/users/${userId}/toggle-status`, {
        is_active,
      });
      return response.data;
    } catch (error) {
  ...
  reducers: {
    ...
    filterUsers: (state, action) => {
      ...
      state.filteredUsers = state.users.filter((user) => {
        ...
        if (password_status) {
          if (password_status === "has_password" && !user.password)
            matches = false;
          if (
            password_status === "first_login" &&
            (!user.ft_password ||
              user.password ||
              user.is_first_login === false)
          )
          ...
          if (
            ...
            (user.password || user.ft_password)
          )
            matches = false;
        }
    ...
    sortUsers: (state, action) => {
      ...
      const usersToSort =
        state.filteredUsers?.length > 0 ? state.filteredUsers : state.users;
    ...
    updateUserInList: (state, action) => {
      ...
      if (state.usersByShop.length > 0) {
        const shopIndex = state.usersByShop.findIndex(
          (user) => user.id === updatedUser.id,
        );
        if (shopIndex !== -1) {
          state.usersByShop[shopIndex] = {
            ...state.usersByShop[shopIndex],
            ...updatedUser,
          };
        }
      ...
      if (state.usersByRole[updatedUser.role]) {
        const roleIndex = state.usersByRole[updatedUser.role].findIndex(
          (user) => user.id === updatedUser.id,
        );
        if (roleIndex !== -1) {
      ...
      if (state.filteredUsers?.length > 0) {
        ...
        if (filteredIndex !== -1) {
          state.filteredUsers[filteredIndex] = {
            ...state.filteredUsers[filteredIndex],
            ...updatedUser,
          };
        }
    ...
    updateStatistics: (state) => {
      ...
      const stats = {
        ...
        byBrand: {},
        passwordStats: {
          with_password: users.filter(
            (user) => user.password && !user.ft_password,
          ).length,
          with_ft_password: users.filter(
            (user) => user.ft_password && !user.password,
          ).length,
    ...
    builder
      ...
      .addCase(requestPasswordReset.rejected, (state, action) => {
        ...
        state.passwordReset.error =
          action.payload?.error || "Failed to send OTP";
      })
      ...
      .addCase(resetPasswordWithOTP.rejected, (state, action) => {
        state.passwordReset.loading = false;
        state.passwordReset.error =
          action.payload?.error || "Failed to reset password";
      })
      ...
      .addCase(resendOTP.rejected, (state, action) => {
        state.passwordReset.loading = false;
        state.passwordReset.error =
          action.payload?.error || "Failed to resend OTP";
      })
      ...
      .addCase(updateUser.fulfilled, (state, action) => {
        ...
        userSlice.caseReducers.updateUserInList(state, {
          payload: updatedUser,
        });
        state.message = action.payload.message;
        userSlice.caseReducers.updateStatistics(state);
      ...
      .addCase(deleteUser.fulfilled, (state, action) => {
        ...
        state.usersByShop = state.usersByShop.filter(
      ...
      .addCase(getAllUsers.fulfilled, (state, action) => {
        ...
        state.users = action.payload.data.map((user) => ({
          ...
          is_first_login:
            user.ft_password && !user.password
              ? true
              : user.is_first_login || false,
        }));
        userSlice.caseReducers.updateStatistics(state);
      ...
      .addCase(getUsersByDistrict.rejected, (state, action) => {
        state.loading = false;
        state.error =
          action.payload?.error || "Failed to fetch users by district";
      });
...
export const selectOtpVerified = (state) =>
  state.user.passwordReset.otpVerified;
export const selectOtpEmail = (state) => state.user.passwordReset.email;
export const selectOtpExpiresAt = (state) =>
  state.user.passwordReset.otpExpiresAt;
...
export const selectBrandAdmins = (brandId) => (state) => {
  return state.user.users.filter(
    (user) => user.role === "brand_admin" && user.brand_id === brandId,
  );
};
